...
8. 附投标人签章栏"""
    ...
    if _match_section(title_lower, ["业绩", "类似项目", "项目经验", "成功案例", "既往项目"]):
        return """【本章节内容规范 — 公司业绩】
本章节展示投标人类似项目业绩，必须按以下结构撰写：

## 公司业绩

### 一、公司业绩一览表
使用Markdown表格列出所有历史合同项目。表格格式如下：
| 序号 | 项目名称 | 采购单位 | 合同金额 | 服务期限 |
|:---|:---|:---|:---|:---|
| 1 | （真实项目名称） | （采购单位） | （合同金额） | （服务期限） |

### 二、项目详细情况
逐个介绍每个项目，格式为：
#### 1、（项目名称）合同
（简要说明项目内容，系统将自动嵌入合同扫描图片，你不需要处理图片）

#### 2、（项目名称）合同
（同上）

严禁编造规则（最高优先级）：
- 项目名称、采购单位、合同金额必须与上文"历史合同业绩数据"完全一致
- 不得编造任何不存在的项目
- 表格中只列历史合同业绩数据中真实存在的项目
- 如果历史合同业绩数据为空，写明"暂无"而非编造"""
